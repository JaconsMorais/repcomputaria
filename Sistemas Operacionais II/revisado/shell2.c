...
char *func_pwd(void) //comando interno pwd
{
    int size;
    char *buf;
...
void func_exit(void){ //comando interno exit
    exit(1);
}
...
void executa_comando_simples(struct command_t *cmd) //execução de um comando
{
...
void executa_comando_pipe(struct command_t *cmd) //execução do comando da pipeline
{
    const char *cmdname = cmd->argv[0];
    glob_t *globbuf = (glob_t *) malloc(sizeof(glob_t));
    globbuf = expande_glob(cmd);
...
int run_pipe(command_t **comandos, int num_args, int num_args0) //pipeline
{
	...
	if(pipe(pd) < 0){
		...
		if (pid < 0) { //erro
			...
			close(pd[0]);
			close(pd[1]);
			executa_comando_pipe(comandos[num_args0-num_args]);
		}
	}
